...
local c; c = { -- each comparison should return error msg if something is wrong, else nil/false
	...
	argIs = function(value, theType, prefixMsg) -- Most comparisons have "..." that are appended to the error message to describe what the test is about
		return type(theType) ~= "string" and ("2nd argument to t.argIs must be value string; received %s"):format(tostring(theType))
	...
	type = function(value, theType, ...)
		return c.argIs(theType, "string") or type(value) ~= theType and appendMsg(("%s (type '%s') is not type '%s'"):format(describe(value), type(value), theType), ...)
	end,
	typeof = function(value, theType, ...)
		return c.argIs(theType, "string") or typeof(value) ~= theType and appendMsg(("%s (typeof '%s') is not typeof '%s'"):format(describe(value), type(value), theType), ...)
	end,
	...
	approxEquals = function(a, b, ...) return c.argIs(a, "number") or c.argIs(b, "number") or math.abs(a - b) >= threshold and actualNotExpected(a, "~~", b, "(approx equals)", ...) end,
	...
	listContains = function(list, value, ...)
		return c.argIs(list, "table", "1st argument")
	...
	listLength = function(list, length, ...)
		return c.argIs(list, "table", "1st argument") or c.argIs(length, "number", "2nd argument") or #list ~= length and appendMsg(("Length of %s is %d instead of %s"):format(describe(list), #list, describe(length)), ...)
	end,
	listsSameLength = function(a, b, ...)
		return c.argIs(a, "table", "1st argument") or c.argIs(b, "table", "2nd argument")
	...
	containsKey = function(theTable, key, ...)
		return c.argIs(theTable, "table", "1st argument") or theTable[key] == nil and appendMsg(("%s does not contain key %s"):format(describe(theTable), describe(key)), ...)
	end,
	...
	tablesEqualRecursive = function(a, b, ...) -- recursive (ignores tables it's already seen before during recursion to avoid infinite loops). Does not consider two keys to be the same if they are different tables, regardless of content.
		local v = c.argIs(a, "table", "1st argument") or c.argIs(b, "table", "2nd argument")
		if v then return v end
		local compare
		local tablesSeen = {}
	...
	isTrue = function(value, func, desc, ...)
		...
		return c.argIs(func, "function", "2nd argument") or not func(value) and appendMsg(("%s is not %s"):format(describe(value), desc and desc or "as expected"), ...) or nil
	end,
